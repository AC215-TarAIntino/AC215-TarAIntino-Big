...
class GeneratedMovie(BaseModel):
    ...
    visual_style: str
    target_audience: str
    inspiration_source: list[str] | None = None
...
class TrailerRequest(BaseModel):
    ...
    movie: GeneratedMovie = Field(..., description="Complete movie data from movie generator")

    target_duration: int = Field(
        default=35, ge=20, le=60, description="Target trailer duration in seconds (20-60)"
    )

    include_narration: bool = Field(default=True, description="Whether to include narration script")
...
class CharacterDesign(BaseModel):
    ...
    brief_identifier: str = Field(
        ...
        examples=[
            "slender woman, late 30s, dark hair",
            "imposing man, grey hair, military bearing",
        ],
    )
...
class TrailerScene(BaseModel):
    ...
    scene_number: int = Field(..., description="Scene order number (1-indexed)")
    ...
    continuity_note: str | None = Field(
        None, description="Optional metadata note about this scene (not sent to models)"
    )
...
class TechnicalSpecs(BaseModel):
    ...
    color_grading: str = Field(
        ...
        examples=[
            "Desaturated with teal shadows and amber highlights",
...
class TrailerBreakdown(BaseModel):
    ...
    total_duration: int = Field(..., description="Total trailer duration in seconds")
    ...
    scenes: list[TrailerScene] = Field(..., description="Ordered list of trailer scenes")

    narration_script: str | None = Field(
        None, description="Full narration script for ElevenLabs (if requested)"
    )

    continuity_guide: str | None = Field(
        None,
...
class TrailerGenerationResponse(BaseModel):
    ...
    trailer: TrailerBreakdown | None = Field(
        None, description="Generated trailer breakdown (null if failed)"
    )
    ...
    generation_time_seconds: float | None = Field(
        None, description="Time taken to generate the trailer breakdown"
    )
...
class MovieAnalysis(BaseModel):
    ...
    key_themes: list[str] = Field(..., description="Key themes to highlight in trailer")
