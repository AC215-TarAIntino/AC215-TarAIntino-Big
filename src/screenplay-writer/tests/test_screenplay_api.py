...
from movie_pipeline.movie_fetcher import MovieFetcherError
...
class TestGenerateMovieEndpoint:
    ...
    def test_generate_movie_success(
        self, mock_fetcher_class, mock_generator_class, client, mock_generated_movie
    ):
        ...
        # Make request
        request_data = {"movie_names": ["Inception", "The Matrix"]}
    ...
    @patch("movie_pipeline.api.MovieGenerator")
    @patch("movie_pipeline.api.MovieFetcher")
    def test_generate_movie_with_model_override(
        self, mock_fetcher_class, mock_generator_class, client, mock_generated_movie
    ):
        ...
        request_data = {"movie_names": ["Inception"], "model": "custom/model"}
        response = client.post("/generate-movie", json=request_data)
    ...
    def test_generate_movie_no_movies_found(self, mock_fetcher_class, client):
        ...
        request_data = {"movie_names": ["NonexistentMovie123"]}
        response = client.post("/generate-movie", json=request_data)
    ...
    def test_generate_movie_invalid_request_too_many_movies(self, client):
        ...
        request_data = {"movie_names": [f"Movie{i}" for i in range(11)]}
        response = client.post("/generate-movie", json=request_data)
    ...
    @patch("movie_pipeline.api.MovieGenerator")
    @patch("movie_pipeline.api.MovieFetcher")
    def test_generate_movie_unexpected_error(
        self, mock_fetcher_class, mock_generator_class, client
    ):
...
class TestAPILogging:
    ...
    def test_logging_on_request(
        self, mock_fetcher_class, mock_generator_class, mock_logger, client, mock_generated_movie
    ):
